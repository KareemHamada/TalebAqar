...
namespace RealEstate.Areas.Admin.Controllers
{
	...
	public class DummyController : Controller
	{
		...
		public DummyController(IUnitOfWork unitOfWork, IMapper mapper, UserManager<ApplicationUser> userManager)
		{
		...
		public async Task<IActionResult> Create(PropertyVM propertyVM, List<IFormFile> images)
		{
			...
			foreach (var image in images)
			{
				...
				{
					var imageUrl = await SaveImageToFileSystem(image, "Properties"); // Method to save the image file and get the URL
		...
		[HttpPost]
		[ValidateAntiForgeryToken]
		public async Task<IActionResult> Edit([FromRoute] int id, PropertyVM propertyVM, List<IFormFile> images)
		{
				...
				try
				{
					...
					foreach (var image in images)
					{
						if (image != null && image.Length > 0)
						{
							...
							{
								ImageUrl = imageUrl,
								PropertyId = id // Set the foreign key
							};
